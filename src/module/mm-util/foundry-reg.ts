import { funcs,  CoreBonus, CoreSystem, Deployable, Environment, Faction, Registry, EntryType, EntryConstructor, ReviveFunc, LiveEntryTypes, RegEntryTypes, RegRef, RegCat, OpCtx, Frame, FrameTrait, License, Manufacturer, Mech, MechSystem, MechWeapon, NpcClass, Npc, Organization, PilotArmor, PilotGear, Pilot, Quirk, Reserve, Sitrep, TagTemplate, Skill, Status, WeaponMod, Talent, StaticReg, RegEnv, CovetousReg, NpcFeature, PilotWeapon } from "machine-mind";
...
// This is what an item-data looks like in foundry. NOTE: Actor data is the same, + some other fields (such as items)
...
// This is what items and actors look like in foundry. NOTE: Actors have some addititional fields for token/item/effect
...
export class FoundryReg extends Registry {
    get_inventory(for_item_id: string): Registry | null {
        let actor = game.actors.get(for_item_id);
        if(actor) {
    ...
    constructor({ for_compendium, for_actor }: { for_compendium?: boolean; for_actor?: Actor | null; } = { for_compendium: false}) {
        super();
        if(for_actor) {
            this.actor = for_actor;
            if(for_compendium) {
                console.error("Ok so honestly I never really thought about what would happen if you tried to contextualize an owned item on a compendium actor. the prospect scares me");
            }
        }
        ...
        function quick_wrapper<T extends EntryType>(for_type: T): EntityCollectionWrapper<T> {
            if([EntryType.MECH, EntryType.PILOT, EntryType.DEPLOYABLE, EntryType.NPC].includes(for_type)) {
                return new GlobalActorWrapper(for_type);
            } else if(for_actor) {
                return new ActorItemWrapper(for_type, for_actor);
            } else {
        ...
        function quick_reviver<T extends EntryType>(for_type: T, clazz: EntryConstructor<T>): ReviveFunc<T> {
        ...
        const do_cat = <T extends EntryType>(for_type: T, clazz: EntryConstructor<T>, defaulter: () => RegEntryTypes<T>) => {
            this.init_set_cat(new FoundryRegCat(this, for_type, defaulter, quick_reviver(for_type, clazz), quick_wrapper(for_type)));
        }
...
abstract class EntityCollectionWrapper<T extends EntryType> {
    ...
    abstract enumerate():  Promise<Array<{
        id: string,
...
class GlobalItemWrapper<T extends EntryType> extends EntityCollectionWrapper<T> {
    ...
    async create(item: RegEntryTypes<T>): Promise<RegRef<T>> {
        ...
        return {
            id: (res.data as any)._id ?? "INVALID",
...
class ActorItemWrapper<T extends EntryType> extends EntityCollectionWrapper<T> {
    ...
    async create(item: RegEntryTypes<T>): Promise<RegRef<T>> {
        ...
        let name = item.name; 
        let res = await this.actor.createOwnedItem({type: this.type, name, data: item});
    ...
    async update(id: string, item: RegEntryTypes<T>): Promise<void> {
        ...
        if(fi) {
            ...
            console.error(`Failed to update item ${id} of type ${this.type} in actor ${this.actor.id} - item not found`);
        }
    }
    ...
    async enumerate(): Promise<Array<{
        ...
        return (this.actor.items.entries as unknown as Array<FoundryRegItem<T>>).map(e => ({
            id: e.data._id,
            item: e.data.data
        }));
    }
...
class CompendiumWrapper<T extends EntryType> extends EntityCollectionWrapper<T> {
    ...
    async create(item: RegEntryTypes<T>): Promise<RegRef<T>> {
        ...
        item = duplicate(item); // better safe than sorry
        //@ts-ignore
        let tmp_item = new Item({type: this.type, name, data: item});
        let intaken = await this.pack.importEntity(tmp_item) as unknown as FoundryRegItem<T>;
